...
var WebClient = function (io) {
    ...
    // Init Run Script List
    this.socket.on('initscripts', function (scripts) {
        var selectedvalue = null;
        var scriptlist = $('#robotinput')[0].options;
        $('#robotinput')[0].options.length = 0;

        scripts.forEach(function (script, index, arr) {
            var option = new Option(script.itemtext, script.itemvalue);
            scriptlist.add(option);
            selectedvalue = script.selected ? script.itemvalue : selectedvalue;
        });

        for (var i = 0; i < scriptlist.length; i++) {
            if (scriptlist[i].value == selectedvalue) {
                scriptlist[i].selected = true;
            }
        }
    });
};
