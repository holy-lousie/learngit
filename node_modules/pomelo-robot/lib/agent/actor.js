...
var script = fs.readFileSync(path.join(process.cwd(), 'app/script', config.script), 'utf8');
...
var Actor = function (conf, aid) {
    ...
    if (typeof conf.server.port === 'number') {
        this.server = conf.server;
    } else {
        var port = conf.server.port[aid % conf.server.port.length];
        this.server = {host: conf.server.host, port: port};
    }

    if (conf.script === config.script) {
        this.script = script;
    } else {
        this.script = fs.readFileSync(path.join(process.cwd(), 'app/script', conf.script), 'utf8');
    }
